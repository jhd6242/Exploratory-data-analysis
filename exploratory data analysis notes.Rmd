...
for (i in 1:dim(data_Matrix_Ordered)[1]){constant_Matrix[i,] <- rep(c(0, 1), each = 5)}
...
```
#### What if we add a 2nd pattern?

```{r}
set.seed(678910)
for (i in 1:40){

	coin_Flip1 <- rbinom(1, size = 1, prob = 0.5)
	coin_Flip2 <- rbinom(1, size = 1, prob = 0.5)
...
svd1 <- svd(scale(data_Matrix_Ordered)); svd2 <- svd(scale(data_Matrix2))
...
```{r}
load("C:/Users/jdecross/Documents/COURSERA/Exploratory-data-analysis/face.rda")
 ...
 * Recently there have been developments to improve the handling/specification of color in plots/graphs/etc.
 * There are functions in R and in external packages that are very handy

#### Colors 1, 2, and 3

#### Default Image Plot in R

### Working with Color in R(part 2)
 ...
 * One package on CRAN that contained interesting/useful color palettes
 * There are 3 types of palettes
      + Sequential
      + Diverging
      + Qualitative

 * Pallet information can be used to conjunctions with the colorRamp() and colorRampPalette()
 ...
 * Careful use of colors in plots/maps/etc. can make it easier for the reader to get what you trying to say(why make it harder?)
